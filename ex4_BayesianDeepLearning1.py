...
# 5.2 训练集准确率
predictions1 = sess.run(pred, feed_dict={x: train_data})
acc1 = np.count_nonzero(predictions1 == np.int32(train_target0.ravel())) / float(train_data.shape[0])
print('训练集准确率：%.2f%s' % (acc1*100, '%'))

# 5.3 测试集准确率
predictions2 = sess.run(pred, feed_dict={x: test_data})
acc2 = np.count_nonzero(predictions2 == np.int32(test_target.ravel())) / float(test_data.shape[0])
print('测试集准确率：%.2f%s' % (acc2*100, '%'))
